...
# Theorem 1 transition point for equal variances
sigma2_bar <- tau_range(edat_train = edat_train, edat_test = edat_test, f_train = f_train, f_test = f_test, Z_train = Z_train, 
                        ...
                        M_merge_linear = M_merge_linear, M_ens_linear = M_ens_linear, cols_re_list = list(cols_re))
sigma2_star <- (ncol(edat_train[[1]])/ncol(Z_train[[1]])) * sigma2_bar[1]

# Corollary 1 performance ratio asymptote
asymptote <- cor1(edat_train = edat_train, edat_test = edat_test, f_train = f_train, f_test = f_test, Z_train = Z_train, 
                  sigma_eps = sigma_eps, wk = wk, lambda_opt = lambda_opt, lambdak_opt = lambdak_opt, learning_rate = learning_rate,
                  M_merge_linear = M_merge_linear, M_ens_linear = M_ens_linear, cols_re_list = list(cols_re))
...
save(err, results, sigma.vals, sigma2_bar, sigma2_star, asymptote,
     file = "Theorem1.RData")
